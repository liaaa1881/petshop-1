...
if (!function_exists('getInitialsEdit')) {
    function getInitialsEdit($name) {
...
// 1. Ambil data lama berdasarkan ID menggunakan Stored Procedure (sp_Karyawan_Read)
if ($id) {
    $sql_get = "{call sp_Karyawan_Read(?, NULL)}";
    $stmt_get = sqlsrv_query($conn, $sql_get, array($id));
...
if (isset($_POST['update']) && $row) {
    ...
    if (!empty($pass_val) && $pass_val !== $pass_konf) {
        ...
        if (empty($error_message)) {
            if (!preg_match('/^(jl\.\s*|jalan\s+)/i', $alamat_ktp) || strlen($alamat_ktp) < 20) {
                $error_message = 'Alamat KTP tidak valid! Harus diawali dengan "Jl." atau "Jalan" dan minimal 20 karakter.';
            } elseif (!preg_match('/^(jl\.\s*|jalan\s+)/i', $alamat_domisili) || strlen($alamat_domisili) < 20) {
                $error_message = 'Alamat Domisili tidak valid! Harus diawali dengan "Jl." atau "Jalan" dan minimal 20 karakter.';
            }
        }
        ...
        if (empty($error_message)) {
            ...
            // Logika Update Foto
            $foto_baru = $row['Foto_Karyawan']; // Default pakai foto lama
            $upload_ok = true;
            ...
            if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
                ...
                if (in_array($ekstensi, $ekstensi_diperbolehkan)) {
                    ...
                    if (!empty($row['Foto_Karyawan']) && file_exists($target_dir . $row['Foto_Karyawan'])) {
                        unlink($target_dir . $row['Foto_Karyawan']);
                    }
            ...
            if ($upload_ok) {
                // PANGGIL STORED PROCEDURE (sp_Karyawan_Update)
                $sql_update = "{call sp_Karyawan_Update(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, ?)}";

                $params = array(
                    $id, $nik, $nama, $jenis_kelamin, $tempat_lahir, $tanggal_lahir, $agama, $status_nikah, $goldar,
                    $telp, $email, $user, $pass, $role, $foto_baru, $alamat_ktp, $alamat_domisili,
                    $kelurahan, $kecamatan, $kota_kabupaten, $provinsi, $kode_pos, $jab, $status_karyawan,
                    $modified_by
                );
                ...
                if ($stmt === false) {
                    ...
                    if ($errors !== null) {
                        ...
                        $error_message = 'Terjadi kesalahan sistem saat memperbarui data staf.';
                    }
                } else {
                    $success_message = 'Berhasil memperbarui data staf!';
                    
                    // Ambil data terbaru hasil update agar form langsung sinkron sebelum halaman berganti
                    $stmt_get = sqlsrv_query($conn, $sql_get, array($id));
                    if ($stmt_get !== false) {
                        $row = sqlsrv_fetch_array($stmt_get, SQLSRV_FETCH_ASSOC);
                    }
    ...
    // Membersihkan output buffer sebelumnya agar respon JSON bersih
    while (ob_get_level()) {
        ob_end_clean();
    }
...
<style>
    ...
    #modalEditKaryawan.show .modal-content-custom {
    ...
    #modalEditKaryawan .modal-dialog {
        max-width: 850px;
        margin-top: 2rem;
        margin-bottom: 2rem;
    }
...
<?php if ($row): ?>
<div class="modal fade" id="modalEditKaryawan" aria-labelledby="modalEditKaryawanLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content modal-content-custom">
            
            <div class="header-bg-edit">
                ...
                <h2 class="fw-bold mb-1" style="letter-spacing: -0.5px; color: white;">Edit Data Staf</h2>
            </div>

            <form id="formEditKaryawan" action="" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-container">
                    ...
                    <div class="section-title d-flex align-items-center">
                        ...
                        Identitas & Data Diri Staf
                    </div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">Nomor Induk Kependudukan (NIK)<span class="text-danger-marker">*</span></label>
                            <input type="text" id="edit_kar_nik" name="nik" class="form-control" maxlength="16" value="<?= htmlspecialchars($row['NIK']) ?>" oninput="this.value = this.value.replace(/[^0-9]/g, '');" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Nama Lengkap Sesuai KTP<span class="text-danger-marker">*</span></label>
                            <input type="text" id="edit_kar_nama" name="nama" class="form-control" value="<?= htmlspecialchars($row['Nama_Karyawan']) ?>" required>
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="edit_tempat_lahir" name="tempat_lahir" class="form-select select2-enable" required>
                                <option value="<?= htmlspecialchars($row['Tempat_Lahir']) ?>" selected><?= htmlspecialchars($row['Tempat_Lahir']) ?></option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <?php 
                                ...
                                if ($row['Tanggal_Lahir'] instanceof DateTime) {
                                    $tgl_lahir_formatted = $row['Tanggal_Lahir']->format('Y-m-d');
                                } else if (!empty($row['Tanggal_Lahir'])) {
                                    $tgl_lahir_formatted = date('Y-m-d', strtotime($row['Tanggal_Lahir']));
                                }
                            ?>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="status_pernikahan" class="form-select select2-nosearch" required>
                                <option value="Belum Kawin" <?= ($row['Status_Pernikahan'] == 'Belum Kawin') ? 'selected' : '' ?>>Belum Kawin</option>
                                <option value="Kawin" <?= ($row['Status_Pernikahan'] == 'Kawin') ? 'selected' : '' ?>>Kawin</option>
                                <option value="Cerai Hidup" <?= ($row['Status_Pernikahan'] == 'Cerai Hidup') ? 'selected' : '' ?>>Cerai Hidup</option>
                                <option value="Cerai Mati" <?= ($row['Status_Pernikahan'] == 'Cerai Mati') ? 'selected' : '' ?>>Cerai Mati</option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="goldar" class="form-select select2-nosearch" required>
                                <option value="A" <?= ($row['Goldar'] == 'A') ? 'selected' : '' ?>>A</option>
                                <option value="B" <?= ($row['Goldar'] == 'B') ? 'selected' : '' ?>>B</option>
                                <option value="AB" <?= ($row['Goldar'] == 'AB') ? 'selected' : '' ?>>AB</option>
                                <option value="O" <?= ($row['Goldar'] == 'O') ? 'selected' : '' ?>>O</option>
                                <option value="-" <?= ($row['Goldar'] == '-') ? 'selected' : '' ?>>-</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" id="edit_alamat_ktp" name="alamat_ktp" class="form-control" value="<?= htmlspecialchars($row['Alamat_KTP'] ?? '') ?>" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Alamat Domisili (Mulai Jl./Jalan, min 20 char)<span class="text-danger-marker">*</span></label>
                            <input type="text" id="edit_alamat_domisili" name="alamat_domisili" class="form-control" value="<?= htmlspecialchars($row['Alamat_Domisili'] ?? '') ?>" required>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select id="edit_provinsi" name="provinsi" class="form-select select2-enable" required>
                                <option value="<?= htmlspecialchars($row['Provinsi']) ?>" selected><?= htmlspecialchars($row['Provinsi']) ?></option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select id="edit_kota_kabupaten" name="kota_kabupaten" class="form-select select2-enable" required>
                                <option value="<?= htmlspecialchars($row['Kota_Kabupaten']) ?>" selected><?= htmlspecialchars($row['Kota_Kabupaten']) ?></option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="edit_kecamatan" name="kecamatan" class="form-select select2-enable" required>
                                <option value="<?= htmlspecialchars($row['Kecamatan']) ?>" selected><?= htmlspecialchars($row['Kecamatan']) ?></option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select id="edit_kelurahan" name="kelurahan" class="form-select select2-enable" required>
                                <option value="<?= htmlspecialchars($row['Kelurahan']) ?>" selected><?= htmlspecialchars($row['Kelurahan']) ?></option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Kode Pos<span class="text-danger-marker">*</span></label>
                            <input type="text" id="edit_kar_kodepos" name="kode_pos" class="form-control" maxlength="5" value="<?= htmlspecialchars($row['Kode_Pos'] ?? '') ?>" oninput="this.value = this.value.replace(/[^0-9]/g, '');" required>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="email" id="edit_kar_email" name="email" class="form-control" value="<?= htmlspecialchars($row['Email'] ?? '') ?>" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Nomor Telepon (Skala Internasional +62/+63)<span class="text-danger-marker">*</span></label>
                            <input type="text" id="edit_kar_telepon" name="telepon" class="form-control" value="<?= htmlspecialchars($row['No_Telepon']) ?>" oninput="this.value = this.value.replace(/[^0-9+]/g, '');" required>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="jabatan" class="form-select select2-nosearch" required>
                                ...
                                <option value="Staff Kasir" <?= ($row['Jabatan'] == 'Staff Kasir') ? 'selected' : '' ?>>Staff Kasir</option>
                                <option value="Staff Grooming" <?= ($row['Jabatan'] == 'Staff Grooming') ? 'selected' : '' ?>>Staff Grooming</option>
                                <option value="Veterinarian" <?= ($row['Jabatan'] == 'Veterinarian') ? 'selected' : '' ?>>Veterinarian (Dokter Hewan)</option>
                                <option value="Asisten Dokter Hewan" <?= ($row['Jabatan'] == 'Asisten Dokter Hewan') ? 'selected' : '' ?>>Asisten Dokter Hewan</option>
                                <option value="Admin Logistik" <?= ($row['Jabatan'] == 'Admin Logistik') ? 'selected' : '' ?>>Admin Logistik</option>
                                <option value="Keeper" <?= ($row['Jabatan'] == 'Keeper') ? 'selected' : '' ?>>Keeper (Penjaga Hewan)</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="status_karyawan" class="form-select select2-nosearch" required>
                                <option value="Tetap" <?= ($row['Status_Karyawan'] == 'Tetap') ? 'selected' : '' ?>>Tetap</option>
                                <option value="Kontrak" <?= ($row['Status_Karyawan'] == 'Kontrak') ? 'selected' : '' ?>>Kontrak</option>
                                <option value="Magang" <?= ($row['Status_Karyawan'] == 'Magang') ? 'selected' : '' ?>>Magang</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label">Nama Pengguna Aplikasi (Min. 5 Karakter)<span class="text-danger-marker">*</span></label>
                            <input type="text" id="edit_kar_user" name="user" class="form-control" value="<?= htmlspecialchars(trim($row['Username'])) ?>" required>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group-custom">
                                <input type="password" id="edit_kar_pass" name="pass" class="form-control" placeholder="Masukkan kata sandi">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group-custom">
                                <input type="password" id="edit_kar_pass_konfirm" name="pass_konfirm" class="form-control" placeholder="Ulangi kata sandi">
                        ...
                        <div class="col-md-12">
                            <div class="avatar-wrapper">
                                ...
                                <div class="flex-grow-1">
                                    <label class="form-label">Ganti Foto Karyawan</label>
                    ...
                    <div class="d-flex justify-content-between align-items-center pt-4 border-top">
                        ...
                        <button type="submit" name="update" class="btn btn-simpan">
                            <i class="fas fa-save me-2"></i>Perbarui Data Staf
                        </button>
                    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        <?php if ($row && empty($success_message)): ?>
            if (modalEl) {
        ...
        const btnSilang = document.querySelector('#modalEditKaryawan .btn-close');
        ...
        const dbPhoto = <?= json_encode(trim($row['Foto_Karyawan'] ?? '')); ?>;
        ...
        if (dbPhoto && dbPhoto !== '') {
            imagePreview.src = '../../assets/uploads/karyawan/' + dbPhoto;
        ...
        fetch('https://www.emsifa.com/api-wilayah-indonesia/api/provinces.json')
            ...
            .then(provinces => {
                ...
                let savedProvinsi = <?= json_encode($row['Provinsi'] ?? '') ?>;
                let activeId = null;
        ...
        function loadRegencies(provId) {
            fetch(`https://www.emsifa.com/api-wilayah-indonesia/api/regencies/${provId}.json`)
                ...
                .then(regencies => {
                    ...
                    let savedKota = <?= json_encode($row['Kota_Kabupaten'] ?? '') ?>;
                    let activeId = null;
        ...
        function loadDistricts(regId) {
            fetch(`https://www.emsifa.com/api-wilayah-indonesia/api/districts/${regId}.json`)
                ...
                .then(districts => {
                    ...
                    let savedKecamatan = <?= json_encode($row['Kecamatan'] ?? '') ?>;
                    let activeId = null;
        ...
        function loadVillages(distId) {
            fetch(`https://www.emsifa.com/api-wilayah-indonesia/api/villages/${distId}.json`)
                ...
                .then(villages => {
                    ...
                    let savedKelurahan = <?= json_encode($row['Kelurahan'] ?? '') ?>;
        ...
        fetch('https://www.emsifa.com/api-wilayah-indonesia/api/provinces.json')
            ...
            .then(provinces => {
                ...
                Promise.all(fetchPromises).then(results => {
                    ...
                    let savedTempatLahir = <?= json_encode($row['Tempat_Lahir'] ?? '') ?>;
        ...
        const nikInput = document.getElementById('edit_kar_nik');
        const dateInput = document.getElementById('tanggal_lahir');
        const form = document.getElementById('formEditKaryawan');

        const alamatKtp = document.getElementById('edit_alamat_ktp');
        const alamatDomisili = document.getElementById('edit_alamat_domisili');
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                // 2. Validasi Panjang NIK (Harus 16 Digit)
                ...
                // 4. Validasi Username Kombinasi Huruf dan Angka tanpa Spasi
                ...
                // 5. Validasi Format Kode Pos (Tepat 5 Digit)
                ...
                // 6. Validasi Format Alamat KTP & Domisili
                ...
                // 7. Validasi Nomor Telepon Skala Internasional
                ...
                // 8. Validasi Format Alamat Email (Surel)
                ...
                // 9. Validasi Kekuatan Password (Hanya jika diisi/ganti baru)
                if (passInput.value !== "") {
                ...
                $.ajax({
                    url: '<?= basename(__FILE__) ?>', // Direct ke file edit ini sendiri
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            Swal.fire({
                                ...
                                // Refresh asinkronus (untuk DataTables / loadData) atau reload halaman lokal
                                if (typeof loadData === 'function') {
                                    loadData();
                                } else if (typeof jQuery !== 'undefined' && typeof $('.dataTable').DataTable === 'function') {
                                    $('.dataTable').DataTable().ajax.reload(null, false);
                                } else if (typeof jQuery !== 'undefined' && typeof $('#tabelKaryawan').DataTable === 'function') {
                                    $('#tabelKaryawan').DataTable().ajax.reload(null, false);
                                } else {
                                    // Fallback aman agar karyawan_read terbaca terupdate
                                    window.location.href = 'karyawan_tampil.php';
                                }
                            });
                        } else {
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        <?php if (!empty($error_message)): ?>
            var modalEditEl = document.getElementById('modalEditKaryawan');
            if (modalEditEl) {
