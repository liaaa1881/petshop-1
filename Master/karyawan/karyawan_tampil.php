...
// --- HANDLER AJAX DETAIL KARYAWAN ---
if (isset($_GET['ajax']) && $_GET['ajax'] == 'detail' && isset($_GET['id'])) {
    $id = $_GET['id'];
    $sql = "{call sp_Karyawan_Read(?, NULL)}";
    ...
    if ($row) {
        if ($row['Tanggal_Lahir'] instanceof DateTime) {
        ...
        echo json_encode(['success' => false, 'message' => 'Staff tidak ditemukan.']);
    }
    exit;
}
...
if (isset($_GET['ajax']) && $_GET['ajax'] == 'search') {
    // Ambil seluruh data aktif lewat Stored Procedure
    $all_employees = [];
    ...
    // Proses filter pencarian dan status di memori PHP
    $filtered = $all_employees;
    if ($search != '') {
        $filtered = array_filter($filtered, function($emp) use ($search) {
            return (stripos($emp['Nama_Karyawan'], $search) !== false) || 
                   (stripos($emp['Username'], $search) !== false);
        });
    }
    if ($status_filter != '') {
        $filtered = array_filter($filtered, function($emp) use ($status_filter) {
            return $emp['Kar_status'] === $status_filter;
        });
    }

    // Urutkan berdasarkan ID Karyawan DESC (Mendukung tipe data numerik maupun kode string alphanumeric)
    usort($filtered, function($a, $b) {
        return (int)$b['ID_Karyawan'] <=> (int)$a['ID_Karyawan'];
    });
                ...
                <td colspan="6" class="text-center py-5">
                    <div class="empty-state animate-fade-in">
                        <div class="empty-icon-wrapper mb-3">
                            <i class="fas fa-user-slash fs-2 text-muted"></i>
                        </div>
                        <h6 class="fw-bold text-dark mb-1">Data staff tidak ditemukan</h6>
    ...
    foreach ($filtered as $row) { 
        $is_aktif = ($row['Kar_status'] == 'Aktif');
        ?>
        <tr class="align-middle employee-row animate-fade-up" id="row-<?= $row['ID_Karyawan'] ?>">
            <td class="text-center fw-bold text-muted"><?= $no++ ?></td>
            <td>
                <div class="d-flex align-items-center">
                    <div class="avatar-container me-3">
                        <?php if (!empty($row['Foto_Karyawan']) && file_exists("../../assets/uploads/karyawan/" . $row['Foto_Karyawan'])): ?>
                            <img src="../../assets/uploads/karyawan/<?= $row['Foto_Karyawan'] ?>" class="avatar" alt="Foto">
                        <?php else: ?>
                            <div class="avatar-initials-circle">
                                <?= getInitials($row['Nama_Karyawan']) ?>
                            </div>
                        <?php endif; ?>
                        <span class="avatar-status-indicator <?= $is_aktif ? 'status-online' : 'status-offline' ?>"></span>
                    </div>
                    <div class="text-truncate" style="max-width: 250px;">
                        <div class="fw-bold text-dark text-truncate fs-6" title="<?= htmlspecialchars($row['Nama_Karyawan']) ?>"><?= htmlspecialchars($row['Nama_Karyawan']) ?></div>
                        <div class="text-muted small text-truncate">@<?= htmlspecialchars($row['Username']) ?></div>
                    </div>
                </div>
            </td>
            <td>
                <span class="badge badge-jabatan text-truncate d-inline-block" style="max-width: 100%;">
                    <?= htmlspecialchars($row['Jabatan']) ?>
                </span>
            </td>
            <td>
                <div class="access-role-tag d-inline-flex align-items-center <?= ($row['Role'] == 'Admin') ? 'role-admin' : 'role-staff' ?>">
                    <span class="role-dot"></span>
                    <span class="fw-bold text-uppercase" style="font-size: 0.75rem; letter-spacing: 0.5px;"><?= htmlspecialchars($row['Role']) ?></span>
                </div>
            </td>
            <td class="text-center">
                <span class="status-pill <?= $is_aktif ? 'pill-aktif' : 'pill-off' ?>" id="status-pill-<?= $row['ID_Karyawan'] ?>">
                    <span class="dot">●</span> <span class="status-text"><?= htmlspecialchars($row['Kar_status']) ?></span>
                </span>
            </td>
            <td class="text-center">
                <div class="d-flex justify-content-center align-items-center action-gap">
                    ...
                    <a href="javascript:void(0)" 
                       ...
                       data-id="<?= $row['ID_Karyawan'] ?>"
                       data-current="<?= htmlspecialchars($row['Kar_status']) ?>"
                       id="toggle-btn-<?= $row['ID_Karyawan'] ?>"
                       title="Ubah Status ke <?= $is_aktif ? 'Non-Aktif' : 'Aktif' ?>">
                        <i class="fas <?= $is_aktif ? 'fa-toggle-on' : 'fa-toggle-off' ?>"></i>
                    </a>

                    <!-- TOMBOL EDIT -->
                    <a href="<?= $is_aktif ? 'karyawan_tampil.php?id=' . $row['ID_Karyawan'] : 'javascript:void(0)' ?>" 
                       class="btn-action btn-edit <?= !$is_aktif ? 'disabled' : '' ?>" 
                       id="edit-btn-<?= $row['ID_Karyawan'] ?>"
                       title="<?= $is_aktif ? 'Edit Data' : 'Karyawan Non-Aktif tidak dapat diedit' ?>">
                    ...
                    <button type="button" class="btn-action btn-hard delete-trigger-btn" 
                            ...
                            data-href="karyawan_hapus.php?id=<?= $row['ID_Karyawan'] ?>"
                            data-id="<?= $row['ID_Karyawan'] ?>"
                            data-action-type="harddelete"
                            data-title="Hapus Karyawan"
                            data-message="Apakah Anda yakin ingin menghapus data staff <b><?= htmlspecialchars($row['Nama_Karyawan']) ?></b> secara permanen?"
...
// Tarik data awal menggunakan Stored Procedure
$all_employees = [];
$sql_sp = "{call sp_Karyawan_Read(NULL, NULL)}";
$query_sp = sqlsrv_query($conn, $sql_sp);
if ($query_sp === false) {
...
$total_k = count($all_employees);
$total_a = count(array_filter($all_employees, function($e) { return $e['Kar_status'] === 'Aktif'; }));
$total_o = count(array_filter($all_employees, function($e) { return $e['Kar_status'] === 'Non-Aktif'; }));
$total_admin = count(array_filter($all_employees, function($e) { return $e['Role'] === 'Admin'; }));
$total_staff = count(array_filter($all_employees, function($e) { return $e['Role'] !== 'Admin'; }));

$unique_jabatans = [];
foreach ($all_employees as $e) {
    if (!empty($e['Jabatan'])) {
        $unique_jabatans[strtolower(trim($e['Jabatan']))] = true;
    }
}
$total_jabatans = count($unique_jabatans);

$pct_aktif = $total_k > 0 ? round(($total_a / $total_k) * 100) : 0;
$pct_off = $total_k > 0 ? round(($total_o / $total_k) * 100) : 0;
$pct_admin = $total_k > 0 ? round(($total_admin / $total_k) * 100) : 0;

// Proses Filter Utama Sesuai URL parameter
$filtered_main = $all_employees;
if ($search != '') {
    $filtered_main = array_filter($filtered_main, function($emp) use ($search) {
        return (stripos($emp['Nama_Karyawan'], $search) !== false) || 
               (stripos($emp['Username'], $search) !== false);
    });
}
if ($status_filter != '') {
    $filtered_main = array_filter($filtered_main, function($emp) use ($status_filter) {
        return $emp['Kar_status'] === $status_filter;
    });
}
...
$paged_employees = array_slice($filtered_main, $offset, $limit);
?>
...
<head>
    ...
    <title>Manajemen Staff | Petshop Pro</title>
    ...
    <style>
        ...
        .card-stat-total:hover { transform: translateY(-6px); box-shadow: 0 20px 35px -10px rgba(79, 70, 229, 0.15); border-color: rgba(79, 70, 229, 0.25); }
        ...
        .card-stat-admin:hover { transform: translateY(-6px); box-shadow: 0 20px 35px -10px rgba(79, 70, 229, 0.15); border-color: rgba(79, 70, 229, 0.25); }
        .card-stat-staff:hover { transform: translateY(-6px); box-shadow: 0 20px 35px -10px rgba(100, 116, 139, 0.15); border-color: rgba(100, 116, 139, 0.25); }
        .card-stat-jabatan:hover { transform: translateY(-6px); box-shadow: 0 20px 35px -10px rgba(6, 182, 212, 0.15); border-color: rgba(6, 182, 212, 0.25); }
        ...
        .icon-box-admin { background: linear-gradient(135deg, #e0e7ff, #818cf8); color: var(--primary-light); }
        .icon-box-staff { background: linear-gradient(135deg, #f1f5f9, #cbd5e1); color: #64748b; }
        .icon-box-jabatan { background: linear-gradient(135deg, #e0f2fe, #bae6fd); color: var(--info-cyan); }
        ...
        .role-admin { background: rgba(79, 70, 229, 0.08); color: var(--primary); }
        .role-admin .role-dot { background-color: var(--primary); }
        .role-staff { background: rgba(148, 163, 184, 0.12); color: var(--slate-700); }
        .role-staff .role-dot { background-color: var(--slate-700); }
        ...
        .btn-edit { background: rgba(79, 70, 229, 0.08); color: var(--primary); }           
        ...
        /* Animasi Masuk Smooth Zoom In */
        #detailKaryawanModal.show .modal-content-custom {
...
<body>
    ...
    <div class="container-fluid px-4 py-5">
        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-5 animate-fade-up">
            <div>
                <h2 class="fw-bold text-dark mb-1">Manajemen Staff ✨</h2>
                <p class="text-muted mb-0">Pantau dan kelola performa serta hak akses tim profesional Anda.</p>
            </div>
            <div class="d-flex flex-wrap gap-3 align-items-center mt-3 mt-md-0">
                ...
                <button type="button" class="btn btn-tambah" data-bs-toggle="modal" data-bs-target="#modalTambahKaryawan">
                    <i class="fas fa-plus-circle me-2"></i> Tambah Data Karyawan
                </button>
            </div>
        </div>
        ...
        <div class="stats-grid">
            ...
            <div class="card card-stat card-stat-total animate-fade-up delay-1">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <p class="stat-label mb-1">Total Staff</p>
            ...
            <div class="card card-stat card-stat-aktif animate-fade-up delay-2">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <p class="stat-label mb-1">Staff Aktif</p>
            ...
            <div class="card card-stat card-stat-nonaktif animate-fade-up delay-3">
                <div class="d-flex justify-content-between align-items-start">
                    ...
                    <div class="stat-icon-box icon-box-off">
            ...
            <!-- TOTAL ADMIN -->
            <div class="card card-stat card-stat-admin animate-fade-up delay-4">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <p class="stat-label mb-1">Administrator</p>
                        <h2 class="stat-value mb-0 text-indigo" id="stat-admin"><?= $total_admin ?></h2>
                    </div>
                    <div class="stat-icon-box icon-box-admin">
                        <i class="fas fa-user-shield"></i>
                    </div>
                </div>
                <div class="stat-progress-bar">
                    <div class="stat-progress-fill" style="width: <?= $pct_admin ?>%; background: var(--primary-light);"></div>
                </div>
            </div>

            <!-- TOTAL STAFF / KARYAWAN -->
            <div class="card card-stat card-stat-staff animate-fade-up delay-5">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <p class="stat-label mb-1">Otoritas Staff</p>
                        <h2 class="stat-value mb-0 text-muted" id="stat-staff"><?= $total_staff ?></h2>
                    </div>
                    <div class="stat-icon-box icon-box-staff">
                        <i class="fas fa-user-tie"></i>
                    </div>
                </div>
                <div class="stat-progress-bar">
                    <div class="stat-progress-fill" style="width: <?= $total_k > 0 ? round(($total_staff / $total_k) * 100) : 0 ?>%; background: #64748b;"></div>
                </div>
            </div>

            <!-- JABATAN TERDISTRIBUSI -->
            <div class="card card-stat card-stat-jabatan animate-fade-up delay-6">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <p class="stat-label mb-1">Klasifikasi Jabatan</p>
                        <h2 class="stat-value mb-0 text-info" id="stat-jabatan"><?= $total_jabatans ?></h2>
                    </div>
                    <div class="stat-icon-box icon-box-jabatan">
        ...
        <div class="glass-card p-4 animate-fade-up delay-5">
               ...
               <table class="table align-middle" style="table-layout: fixed; width: 100%;">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 30%;">Profil Karyawan</th>
                            <th style="width: 25%;">Jabatan</th>
                            <th style="width: 15%;">Hak Otoritas</th>
                    ...
                    <tbody id="employee-tbody">
                        <?php
                        $no = 1 + $offset;
                        if (empty($paged_employees)) {
                            echo '<tr><td colspan="6" class="text-center py-5 text-muted">Tidak ada data staff pada halaman ini...</td></tr>';
                        } else {
                            foreach ($paged_employees as $row) { 
                                $is_aktif = ($row['Kar_status'] == 'Aktif');
                            ?>
                            <tr class="align-middle employee-row" id="row-<?= $row['ID_Karyawan'] ?>">
                                <td class="text-center fw-bold text-muted"><?= $no++ ?></td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="avatar-container me-3">
                                            <?php if (!empty($row['Foto_Karyawan']) && file_exists("../../assets/uploads/karyawan/" . $row['Foto_Karyawan'])): ?>
                                                <img src="../../assets/uploads/karyawan/<?= $row['Foto_Karyawan'] ?>" class="avatar shadow-sm border border-light" alt="Foto">
                                            <?php else: ?>
                                                <div class="avatar-initials-circle">
                                                    <?= getInitials($row['Nama_Karyawan']) ?>
                                                </div>
                                ...
                                <td>
                                    <span class="badge badge-jabatan text-truncate d-inline-block" style="max-width: 100%;">
                                        <i class="fas fa-sitemap me-1 text-indigo"></i> <?= htmlspecialchars($row['Jabatan'] ?: 'Staff') ?>
                                    </span>
                                </td>
                                <td>
                                    <div class="access-role-tag d-inline-flex align-items-center <?= ($row['Role'] == 'Admin') ? 'role-admin' : 'role-staff' ?>">
                                ...
                                <td class="text-center">
                                    <span class="status-pill <?= $is_aktif ? 'pill-aktif' : 'pill-off' ?>" id="status-pill-<?= $row['ID_Karyawan'] ?>">
                                        <span class="dot">●</span> <span class="status-text"><?= htmlspecialchars($row['Kar_status'] ?: 'Non-Aktif') ?></span>
                                    </span>
                                </td>
                                <td class="text-center">
                                    <div class="d-flex justify-content-center align-items-center action-gap">
                                        ...
                                        <a href="javascript:void(0)" 
                                           class="btn-action toggle-status-btn <?= $is_aktif ? 'btn-status-aktif' : 'btn-status-off' ?>" 
                                           data-id="<?= $row['ID_Karyawan'] ?>"
                                           data-current="<?= htmlspecialchars($row['Kar_status'] ?: 'Non-Aktif') ?>"
                                           id="toggle-btn-<?= $row['ID_Karyawan'] ?>"
                                           title="Ubah Status ke <?= $is_aktif ? 'Non-Aktif' : 'Aktif' ?>">
                                            <i class="fas <?= $is_aktif ? 'fa-toggle-on' : 'fa-toggle-off' ?>"></i>
                                        </a>
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mt-4 px-2">
                <div class="text-muted small mb-2 mb-md-0">
                    Menampilkan <strong><?= min($offset + 1, $total_records) ?></strong> sampai <strong><?= min($offset + $limit, $total_records) ?></strong> dari total <strong><?= $total_records ?></strong> staff tersaring.
                </div>
                <nav>
            ...
            <div class="modal-content modal-content-custom border-0">
                ...
                <div class="modal-header-centered">
                    ...
                    <div class="d-flex flex-column align-items-center text-center">
                        ...
                        <h2 class="fw-bold mb-1 text-white" id="detail-nama" style="letter-spacing:-0.5px;">Nama Karyawan</h2>
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            <div class="card border-0 p-3 shadow-sm h-100" style="border-radius: 16px; background:#fff;">
                                <h6 class="fw-bold text-primary mb-3"><i class="fas fa-briefcase me-2"></i>Status Pekerjaan & Akses</h6>
                                <table class="table table-borderless table-sm mb-0 small">
                                    <tr><td class="text-muted" style="width:40%;">NIK</td><td class="fw-bold text-dark" id="detail-nik">-</td></tr>
                                    <tr><td class="text-muted">Jabatan</td><td class="fw-bold text-dark" id="detail-jabatan">-</td></tr>
                                    <tr><td class="text-muted">Status Hubungan</td><td class="fw-bold text-dark" id="detail-status-karyawan">-</td></tr>
                                    <tr><td class="text-muted">Role Aplikasi</td><td class="fw-bold text-dark text-uppercase" id="detail-role">-</td></tr>
                        ...
                        <div class="col-md-6">
                            <div class="card border-0 p-3 shadow-sm h-100" style="border-radius: 16px; background:#fff;">
                                ...
                                <table class="table table-borderless table-sm mb-0 small">
                                    ...
                                    <tr><td class="text-muted">Agama</td><td class="fw-bold text-dark" id="detail-agama">-</td></tr>
                                    <tr><td class="text-muted">Status Pernikahan</td><td class="fw-bold text-dark" id="detail-status-pernikahan">-</td></tr>
                                    <tr><td class="text-muted">Golongan Darah</td><td class="fw-bold text-dark" id="detail-goldar">-</td></tr>
                                </table>
                            </div>
                        </div>
                        ...
                        <div class="col-12">
                            <div class="card border-0 p-3 shadow-sm" style="border-radius: 16px; background:#fff;">
                                ...
                                <div class="row g-2 small">
                                    <div class="col-md-6"><span class="text-muted d-block">Alamat KTP</span><strong class="text-dark" id="detail-alamat-ktp">-</strong></div>
                                    <div class="col-md-6"><span class="text-muted d-block">Alamat Domisili</span><strong class="text-dark" id="detail-alamat-domisili">-</strong></div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg" style="border-radius: 24px; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px);">
                ...
                <div class="modal-body py-4 px-4 text-center">
                    <div class="mb-3 d-inline-flex align-items-center justify-content-center" style="width: 60px; height: 60px; border-radius: 50%; background: rgba(239, 68, 68, 0.1);">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function refreshTable() {
                ...
                const url = `karyawan_tampil.php?${urlParams.toString()}`;
                ...
                fetch(url)
                    ...
                    .then(html => {
                        ...
                        const stats = ['stat-total', 'stat-aktif', 'stat-off', 'stat-admin', 'stat-staff', 'stat-jabatan'];
                        stats.forEach(id => {
                        ...
                        const progressBars = ['progress-aktif', 'progress-off'];
            ...
            function performSearchAndFilter() {
                ...
                fetch(`karyawan_tampil.php?ajax=search&search=${encodeURIComponent(queryValue)}&status_filter=${encodeURIComponent(currentStatusFilter)}`)
                    .then(response => response.text())
                    .then(html => {
            ...
            // AJAX Detail Karyawan (Tombol Lihat)
            tbody.addEventListener('click', function(e) {
                ...
                if (viewBtn) {
                    ...
                    fetch(`karyawan_tampil.php?ajax=detail&id=${id}`)
                        .then(response => response.json())
                        .then(res => {
                            if (res.success) {
                                const d = res.data;
                                
                                document.getElementById('detail-nik').textContent = d.NIK || '-';
                                document.getElementById('detail-nama').textContent = d.Nama_Karyawan || '-';
                                ...
                                document.getElementById('detail-agama').textContent = d.Agama || '-';
                                document.getElementById('detail-status-pernikahan').textContent = d.Status_Pernikahan || '-';
                                document.getElementById('detail-goldar').textContent = d.Goldar || '-';
                                document.getElementById('detail-email').textContent = d.Email || '-';
                                document.getElementById('detail-telp').textContent = d.No_Telepon || '-';
                                document.getElementById('detail-jabatan').textContent = d.Jabatan || '-';
                                document.getElementById('detail-status-karyawan').textContent = d.Status_Karyawan || '-';
                                document.getElementById('detail-role').textContent = d.Role || '-';
                                document.getElementById('detail-username').textContent = '@' + (d.Username || '-');
                                document.getElementById('detail-alamat-ktp').textContent = d.Alamat_KTP || '-';
                                document.getElementById('detail-alamat-domisili').textContent = d.Alamat_Domisili || '-';
                                ...
                                if (d.Foto_Karyawan && d.Foto_Karyawan !== '') {
                                    modalAvatarContainer.innerHTML = `<img src="../../assets/uploads/karyawan/${d.Foto_Karyawan}" style="width:90px; height:90px; border-radius:18px; object-fit:cover; border:3px solid #fff; box-shadow:0 8px 20px rgba(0,0,0,0.15);">`;
                                } else {
                                    const initials = getInitialsSupplierJs(d.Nama_Karyawan);
                                ...
                                const detailModal = new bootstrap.Modal(document.getElementById('detailKaryawanModal'));
                                detailModal.show();
                            } else {
            ...
            tbody.addEventListener('click', function (e) {
                ...
                if (toggleBtn) {
                    ...
                    fetch(`karyawan_toggle_status.php?id=${id}&current=${encodeURIComponent(currentStatus)}`)
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                                ...
                                if (editBtn) {
                                    if (newStatus === 'Aktif') {
                                        ...
                                        editBtn.setAttribute('href', `karyawan_tampil.php?id=${id}`);
                                    } else {
                                ...
                                showToast(`Status karyawan berhasil diubah ke ${newStatus}`, 'success');
                                ...
                                showToast('Gagal memperbarui status karyawan.', 'danger');
                            }
                        })
                        .catch(error => {
            ...
            if (confirmExecuteBtn) {
                confirmExecuteBtn.addEventListener('click', function(e) {
                    ...
                    fetch(deleteUrl, { 
                        ...
                        .then(response => {
                            // Cek status HTTP respon dari karyawan_hapus.php
                            ...
                            if (data.success) {
                                ...
                                if (targetRow) {
                                    ...
                                    setTimeout(() => {
                                        ...
                                        refreshTable();
                                        showToast(data.message || 'Staff berhasil dinonaktifkan', 'success');
                                    }, 400);
                                } else {
                                    refreshTable();
                                    showToast(data.message || 'Staff berhasil dinonaktifkan', 'success');
                                ...
                                showToast(data.message || 'Gagal menangguhkan staff.', 'danger');
                            }
                        })
                        .catch(error => {
                            console.error('Error Action AJAX:', error);
                            showToast(error.message || 'Terjadi kesalahan saat memproses data.', 'danger');
                        })
                        .finally(() => {
    ...
    <?php include 'karyawan_tambah.php'; ?>
    <?php include 'karyawan_edit.php'; ?>
</body>
</html>
