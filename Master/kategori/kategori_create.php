                ...
                try {
                    ...
                    if (data.status === 'success') {
                        Swal.fire({
                            ...
                            timer: 2000,              // Penutupan otomatis selama 2 detik
...
// Menggunakan penanganan AJAX POST untuk kategori
// Handler HANYA berjalan jika ini adalah proses CREATE (tidak ada parameter 'id' di dalam payload POST)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Nama_Kategori']) && !isset($_POST['id'])) {
    header('Content-Type: application/json');
    ...
    // Data Audit & Parameter Tambahan
    $kat_status  = 'Aktif'; 
    $foto_barang = null; // Default null untuk parameter @Foto_Barang di SP
    $created_by  = $_SESSION['username'] ?? $_SESSION['Username'] ?? $_SESSION['nama'] ?? 'Admin';

    // Proses Upload Foto Kategori
    $foto_baru = null; 
    $upload_ok = true;

    // 1. Validasi Sisi Server: Nama Kategori Hanya Alphabet & Spasi
    if (!preg_match('/^[a-zA-Z\s]+$/', $nama)) {
        $error_message = 'Nama kategori hanya diperbolehkan berisi huruf alfabet dan spasi!';
        $upload_ok = false;
    }
    // 2. Validasi Sisi Server: Minimal Deskripsi 20 Karakter
    elseif (strlen(trim($deskripsi)) < 20) {
        ...
        if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
            ...
            $upload_ok = false;
            $error_message = 'Foto kategori wajib diunggah!';
        }
    }

    if ($upload_ok) {
        // PEMANGGILAN STORED PROCEDURE (sp_Kategori_Create)
...
<!-- STYLE KHUSUS MODAL TAMBAH KATEGORI (TEMA VIOLET) -->
<style>
    :root {
    ...
    /* Geser modal & SweetAlert2 ke kanan pada layar desktop untuk mengimbangi sidebar */
    @media (min-width: 992px) {
        #modalTambahKategori {
            padding-left: 260px !important; 
        }
    ...
    #modalTambahKategori.show .modal-content-custom-kategori {
    ...
    #modalTambahKategori .modal-dialog {
        max-width: 850px;
        margin-top: 2rem;
        margin-bottom: 2rem;
    }
...
<!-- MODAL CONTAINER TAMBAH KATEGORI -->
<div class="modal fade" id="modalTambahKategori" tabindex="-1" aria-labelledby="modalTambahKategoriLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content modal-content-custom modal-content-custom-kategori">
            ...
            <div class="header-bg-kategori">
                ...
                <h2 class="fw-bold mb-1" style="letter-spacing: -0.5px; color: white;">Tambah Kategori Baru</h2>
                <p class="opacity-75 mb-0" style="font-size: 0.95rem; color: white;">Kelompokkan klasifikasi barang dan jasa layanan Petshop Pro</p>
            </div>

            <form id="formTambahKategori" action="" method="POST" enctype="multipart/form-data">
                <div class="form-container">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" id="cat_nama" name="Nama_Kategori" class="form-control" placeholder="Contoh: Makanan Kucing, Grooming..." required>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="Tipe_Kategori" id="cat_tipe" class="form-select select2-enable-kategori" required>
                                <option value="" disabled selected>Pilih Tipe Kategori...</option>
                                <option value="Barang">Barang (Produk Fisik)</option>
                                <option value="Layanan">Layanan (Jasa / Servis)</option>
                            </select>
                        </div>
                        <div class="col-md-12">
                            <label class="form-label">Deskripsi Singkat (Minimal 20 Karakter)<span class="text-danger-marker">*</span></label>
                            <textarea name="Deskripsi" id="cat_deskripsi" class="form-control" rows="3" placeholder="Tuliskan keterangan fungsional mengenai kategori ini..." required></textarea>
                            <div class="d-flex justify-content-between mt-1">
                                <span class="text-muted" style="font-size: 0.75rem;">Harap tuliskan informasi deskripsi yang representatif.</span>
                                <span id="char-counter" class="text-muted" style="font-size: 0.75rem;">0 / 20 karakter minimum</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-12">
                            <div class="avatar-wrapper">
                                <div id="avatar-container-kat" class="avatar-preview-circle">
                                    <span id="avatar-initials-kat">?</span>
                                    <img id="avatar-image-preview-kat" src="" alt="Pratinjau Visual">
                                </div>
                                <div class="flex-grow-1">
                                    <label class="form-label">Unggah Foto / Ikon Kategori<span class="text-danger-marker">*</span></label>
                                    <input type="file" id="foto-input-kat" name="foto" class="form-control" accept="image/*" required>
                                    <div class="form-text text-muted mt-1" style="font-size:0.8rem;">Format didukung: <strong>JPG, JPEG, PNG, WEBP</strong>. Maksimal ukuran berkas: <strong>2 MB</strong>.</div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center pt-4 border-top">
                        ...
                        <button type="submit" name="simpan" class="btn btn-simpan-kategori">
...
<!-- JAVASCRIPT: PREVIEW FOTO, INISIAL GENERATOR & VALIDASI KATEGORI -->
<script>
document.addEventListener("DOMContentLoaded", function() {
    ...
    $('.select2-enable-kategori').select2({
        dropdownParent: $(document.body),
        width: '100%',
        minimumResultsForSearch: Infinity, // Hilangkan kolom pencarian untuk opsi yang sedikit
    ...
    const namaInput = document.getElementById('cat_nama');
    const initialsSpan = document.getElementById('avatar-initials-kat');
    const imagePreview = document.getElementById('avatar-image-preview-kat');
    const fileInput = document.getElementById('foto-input-kat');
    const deskripsiInput = document.getElementById('cat_deskripsi');
    ...
    // VALIDASI INPUT: Cegah pengetikan karakter selain alfabet dan spasi pada Nama Kategori
    if (namaInput) {
        namaInput.addEventListener('input', function() {
            // Hilangkan semua karakter non-alfabet dan non-spasi secara real-time
            this.value = this.value.replace(/[^a-zA-Z\s]/g, '');
            updateInitials();
        });
    }

    // PENGHITUNG KARAKTER DESKRIPSI SECARA REAL-TIME
    if (deskripsiInput) {
    ...
    // SUBMIT DATA VIA AJAX UNTUK MENCEGAH REFRESH HALAMAN & KEHILANGAN INPUT DATA
    if (form) {
        form.addEventListener('submit', function(e) {
            e.preventDefault();
            ...
            // 3. Validasi Keberadaan Foto Kategori
            if (!fileInput.files || fileInput.files.length === 0) {
                Swal.fire({
                    icon: 'error',
                    title: 'Foto Kategori Wajib Diunggah',
                    text: 'Silakan unggah berkas foto / ikon kategori terlebih dahulu.',
                    confirmButtonColor: '#8b5cf6'
                });
                return;
            }
